...
namespace Code2Xml.Core.Location {
    ...
    public struct CodeLocation : IEquatable<CodeLocation>, IComparable<CodeLocation> {
        public static CodeLocation Nil = new CodeLocation { Line = -1, Position = -1 };
        ...
        public static CodeLocation ConvertFromIndex(
                string code, int index, CodeLocation startLocation, int startIndex) {
            return ConvertFromIndex(
                    code, index, startLocation.Line, startLocation.Position, startIndex);
        }
        ...
        public static CodeLocation ConvertFromIndex(
                StructuredCode code, int index, CodeLocation startLocation) {
            return code.GetLocation(index, startLocation);
        }

        private static CodeLocation ConvertFromIndex(
                string code, int endIndex, int startLine, int startPosition, int startIndex) {
            var i = startIndex;
            for (; i < endIndex; i++) {
                if (code[i] != '\n') {
                    startPosition++;
