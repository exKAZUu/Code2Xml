...
namespace Code2Xml.Learner.Core.Learning.Experiments {
	public abstract class Experiment {
		...
		private void WriteFeatureStrings(
				LearningExperiment exp, ICollection<string> projectPaths, LearningResult learningResult) {
			var featureWriter =
					CreateWriter(exp.GetType().Name + "_feature_" + projectPaths.Count() + ".txt");
			var acceptingFeature =
					learningResult.Classifier.GetAllAcceptingFeatureStrings(learningResult.FeatureEncoder);
			var rejectingFeatures =
					learningResult.Classifier.GetAllRejectingFeatureStrings(learningResult.FeatureEncoder);
			featureWriter.WriteLine("################ Accepting Features ################");
			for (int i = 0; i < acceptingFeature.Count; i++) {
				featureWriter.WriteLine("######## Group: " + (i + 1) + " ########");
				featureWriter.WriteLine(learningResult.Classifier.GroupPaths[i]);
				featureWriter.WriteLine();
				foreach (var feature in acceptingFeature[i]) {
					featureWriter.WriteLine(feature);
				}
			}
			featureWriter.WriteLine("################ Rejecting Features ################");
			for (int i = 0; i < rejectingFeatures.Count; i++) {
				featureWriter.WriteLine("######## Group: " + (i + 1) + " ########");
				featureWriter.WriteLine(learningResult.Classifier.GroupPaths[i]);
				featureWriter.WriteLine();
