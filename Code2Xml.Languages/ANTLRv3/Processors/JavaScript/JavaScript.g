...
statementEnd
    ...
    | LineComment
    | EOF!
    | { input.LA(-1) == RBRACE || input.LA(1) == RBRACE }?
    ;
...
caseBlock
    : '{' (LT!* caseClause)* (LT!* defaultClause (LT!* caseClause)*)? LT!* RBRACE
    ;
...
objectLiteral
    : '{' LT!* propertyNameAndValue? (LT!* ',' (LT!* propertyNameAndValue)?)* LT!* RBRACE
    ;
...
fragment ContinuousLineCharacter
    : '\\' LT
    ;
...
fragment RegularExpressionInnerChar
    : ~ ( LT | '\\' | ']' )
    | '\\' ~LT
    ;
