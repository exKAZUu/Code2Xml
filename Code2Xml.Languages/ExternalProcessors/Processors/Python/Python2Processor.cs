...
namespace Code2Xml.Languages.ExternalProcessors.Processors.Python {
    ...
    public class Python2Processor : ProcessorUsingExternalParserAndOwnCodeGenerator {
        ...
        public Python2Processor()
                : this("\n") {}

        public Python2Processor(string newLine)
                : this(newLine, ExternalProgramUtils.GetPythonPath("2") ?? "python2") {}

        public Python2Processor(string newLine, string processorPath)
                : base(newLine, ".py") {
            _processorPath = processorPath;

            ParaibaFile.WriteIfDifferentSize(PrivateXmlGeneratorArguments[0], Resources.ast2xml);
        }
