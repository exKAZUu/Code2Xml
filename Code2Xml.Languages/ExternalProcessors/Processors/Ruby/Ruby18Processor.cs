...
namespace Code2Xml.Languages.ExternalProcessors.Processors.Ruby {
    ...
    public class Ruby18Processor : ProcessorUsingExternalProcessor {
        ...
        public Ruby18Processor()
                : this(ExternalProgramUtils.GetRubyPath() ?? "ruby") {}

        public Ruby18Processor(string processorPath)
                : base(".rb") {
            _processorPath = processorPath;

            ParaibaFile.WriteIfDifferentSize(PrivateXmlGeneratorArguments[0], Resources.ruby182xml);
            ParaibaFile.WriteIfDifferentSize(PrivateCodeGeneratorArguments[0], Resources.xml2ruby18);
